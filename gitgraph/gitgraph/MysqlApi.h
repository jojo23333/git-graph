...
public:
	...
	uint32_t write_user(gitgraph *g);
	//write all the user in the database into graph's user vector
	//return total amount of user
	uint32_t write_following(gitgraph *g, string username);
	uint32_t write_follower(gitgraph *g, string username);
	//write a single user's followers and followings
	//return total amount
	bool write_all(gitgraph &g);
	//this method rewrite graph from database
};

uint32_t mysql_api::write_user(gitgraph *g)
{
...
uint32_t mysql_api::write_following(gitgraph *p, string username)
{
...
uint32_t mysql_api::write_follower(gitgraph *p, string username)
{
	boost::scoped_ptr< sql::ResultSet > res(stmt->executeQuery(\
...
bool mysql_api::write_all(gitgraph &g)
{
	write_user(&g);
